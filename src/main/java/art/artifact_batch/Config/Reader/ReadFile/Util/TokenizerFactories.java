package art.artifact_batch.Config.Reader.ReadFile.Util;
...
public final class TokenizerFactories {
    ...
    public static LineTokenizer getLineTokenizer(ReadProperties props) {
        ...
        if (FileType.DELIMITED.equals(props.getFileType())) {
            return createDelimitedTokenizer(props);
        }else if (FileType.FIXED_LENGTH.equals(props.getFileType())) {
            return createFixedLengthTokenizer(props);
        }else {

            throw new ConfigurationException("Unsupported file type for default tokenizers: " + props.getFileType() +
