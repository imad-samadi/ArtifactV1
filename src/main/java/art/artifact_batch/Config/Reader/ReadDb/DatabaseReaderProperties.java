...
public class DatabaseReaderProperties {
    ...
    public void initialize() {
        ...
        try {
            this.resolvedTargetType = Class.forName(targetModel);
        } catch (ClassNotFoundException e) {
            throw new ConfigurationException("Invalid target class: " + targetModel, e);
        }
        ...
        if (DatabaseType.JDBC.equals(this.databaseType)) {
            ...
            log.info("JDBC Reader configured for target DTO: {}", targetModel);

        } else if (DatabaseType.REPOSITORY.equals(this.databaseType)) {

            // Log details or warnings about parameter usage
            log.info("REPOSITORY Reader configured for entity: {}, repository bean: '{}', method: '{}'", targetModel, serviceBeanName, serviceMethodName);
...
// ----------------------------------
    ...
    public String getActualTableName() {
        if (!DatabaseType.JDBC.equals(databaseType)) {
            return null;
        }
    ...
    public String getActualSelectClause() {
        ...
        if (columnNames.isEmpty()) {
            throw new ConfigurationException("JDBC: Could not derive select clause for DTO " + targetModel);
        }
    ...
    public Map<String, org.springframework.batch.item.database.Order> getActualSortOrders() {
        ...
        if (!sortKeys.isEmpty()) {
            ...
            log.info("JDBC: No 'sortKeys' provided for {}. Attempting default sort.", targetModel);
     ...
     */
    @AssertTrue(message = "When Type is REPOSITORY, serviceBeanName and serviceMethodName must be set.")
    ...
    public Sort getRepositorySpringSort() {
        if (!DatabaseType.REPOSITORY.equals(databaseType) || repositorySorts == null || repositorySorts.isEmpty()) {
    ...
    private boolean containsNamedParameters(String clause) {
        if (!DatabaseType.JDBC.equals(databaseType) || clause == null) return false;
